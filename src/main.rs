...
use std::{collections::HashMap, ops, thread, vec::Vec};
use winit::{
...
impl Camera {
    fn create(pos: Vec3, size: f32, aspect_ratio: f32, lens_plane: f32) -> Camera {
    ...
    fn get_world_pos(&self, screen_x: f32, screen_y: f32) -> Vec3 {
        (self.x * screen_x * self.size * self.aspect_ratio)
            + (self.y * screen_y * self.size)
            + self.pos
            + (self.z * self.lens_plane)
    }
}
...
impl Plane {
    fn create(normal: Vec3, d: f32, material_id: u64) -> Plane {
        Plane {
            normal,
            d,
            material_id,
    ...
    fn has_contact(&self, dir: Vec3) -> bool {
        self.normal.dot(dir * -1.0) > 0.0
    }
...
impl Sphere {
    fn create(pos: Vec3, radius: f32, material_id: u64) -> Sphere {
        Sphere {
            pos,
            radius,
            material_id,
...
fn draw_rows(
    ...
    for y in 0..row_count {
        let screen_y = ((y + row_start) as f32 / height as f32) - 0.5;
        for x in 0..width {
            let mut color = Vec3::create(0.0, 0.0, 0.0);
            let screen_x = (x as f32 / width as f32) - 0.5;

            const RAYS_PER_PIXEL: u32 = 32;
            for _ in 0..RAYS_PER_PIXEL {
                let screen_x = screen_x + ((rand::random::<f32>() - 0.5) / (width as f32));
                let screen_y = screen_y + ((rand::random::<f32>() - 0.5) / (height as f32));

                let film_point = camera.get_world_pos(screen_x, screen_y);

                //color = color + (cast_ray(&world, camera.pos, (film_point - camera.pos).normalize()) * (1.0 / RAYS_PER_PIXEL as f32));
                color = color
                    + (cast_ray(&world, film_point, camera.z) * (1.0 / RAYS_PER_PIXEL as f32));
            }

            if color.x <= 0.0031308 {
                color.x = color.x * 12.92;
            } else {
                color.x = (1.055 * color.x.powf(1.0 / 2.4)) - 0.055
            }
            if color.y <= 0.0031308 {
                color.y = color.y * 12.92
            } else {
                color.y = (1.055 * color.y.powf(1.0 / 2.4)) - 0.055
            }
            if color.z <= 0.0031308 {
                color.z = color.z * 12.92
            } else {
                color.z = (1.055 * color.z.powf(1.0 / 2.4)) - 0.055
            }
            let i = ((y * width) + x) as usize;
            buffer[i] = (color.z * 255.0) as u32
...
fn main() {
    ...
    window.set_inner_size(LogicalSize::new(1600, 900));
    let mut graphics_context = unsafe { GraphicsContext::new(window) }.unwrap();

    let (width, height) = {
        let size: PhysicalSize<u32> = graphics_context.window().inner_size();
        (size.width, size.height)
    };

    let mut camera = Camera::create(
        Vec3::create(0.0, 6.0, 6.0),
        5.0,
    ...
    world.add_material(
        ...
        Vec3::create(0.392, 0.584, 0.929),
        0.0,
    );

    let sun = world.add_material(
        Vec3::create(0.0, 0.0, 0.0),
    ...
    let mat2 = world.add_material(
        Vec3::create(0.5, 0.7, 0.3),
        Vec3::create(0.0, 0.0, 0.0),
        0.95,
    );
    let mat3 = world.add_material(
        Vec3::create(0.95, 0.95, 0.95),
        Vec3::create(0.0, 0.0, 0.0),
        1.0,
    );
    let mat4 = world.add_material(
        Vec3::create(0.8, 0.8, 0.8),
        Vec3::create(0.6, 0.0, 0.0),
        0.6,
    );
    ...
    let buffer: Arc<Mutex<Vec<u32>>> = Arc::new(Mutex::new(vec![0; (width * height) as usize]));
    let camera = Arc::new(camera);
    let world = Arc::new(world);

    let mut row_index: u32 = 0;
    let mut row_count = height / 8;
    for _ in 0..8 {
        let camera = Arc::clone(&camera);
        let world = Arc::clone(&world);
        let buffer = Arc::clone(&buffer);
        if row_index + row_count > height {
            row_count = height - row_index;
        }

        thread::spawn(move || {
            let mut per_thread_buffer: Vec<u32> = vec![0; (row_count * width) as usize];
            draw_rows(
                &world,
                &camera,
                &mut per_thread_buffer,
                width,
                height,
                row_index,
                row_count,
            ...
            let dst_slice = &mut buffer
                [(row_index * width) as usize..((row_count + row_index) * width) as usize];
            dst_slice.copy_from_slice(&per_thread_buffer[..]);
            drop(buffer);
        });
        row_index += row_count;
    }
